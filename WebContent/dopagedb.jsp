...
<%
	String strpage = request.getParameter("currentpage"); //获取DBlist发送过来的页面页码
	String pcsname = request.getParameter("pcsname"); //获取DBlist发送过来的查询开始时间
	String datastart = request.getParameter("datastart"); //获取DBlist发送过来的查询开始时间
	String dataend = request.getParameter("dataend"); //获取DBlist发送过来的查询结束时间
	...
	if (datastart != null && dataend != null) {
		...
		System.out.println("本次连接的是数据表是：" + pcsname);
		...
		ResultSet rs = db.getmatchPageRs(pcsname, datastart, dataend); //调用getmatchPageRs函数   获取符合条件的当前页的数据
		...
		response.sendRedirect("DBlist.jsp"); //重定向到 index.jsp页面     	 
		...
		db.setPageInfo(pcsname, currentpage); //调用DB类中的 setPageInfo函数 获取 按当前 “获取当前指针所指记录的行号，即总记录数 num_rs ”和总页数pages_al  		 
		ResultSet rs = db.getPageRs(pcsname); //调用getPageRs函数   获取当前页的数据
		session.setAttribute("db", db); //保存 db到session
		session.setAttribute("pageresultset", rs); //保存获取的数据
		response.sendRedirect("DBlist.jsp"); //重定向到 index.jsp页面     	 
